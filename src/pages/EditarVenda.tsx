...
useEffect(() => {
  ...
  async function carregarVenda(vendaId: string) {
  ...
  setCliente(venda.cliente_id ?? CONSUMIDOR_FINAL)
  setDataVenda(
    venda.data_venda
      ? venda.data_venda.split('T')[0]
      : '',
  )
  setTipoVenda(venda.tipo_venda ?? 'BALCAO')
  setDescontoGeral(Number(venda.desconto ?? 0))
  setAcrescimo(Number(venda.acrescimo ?? 0))
...
await vendasService.atualizarVenda(id!, {
        ...
        observacoes_pagamento:observacoesPagamento || undefined,
